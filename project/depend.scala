...
object depend {
  ...
  val sawsVersion = "1.2.1-20150309055210-e068cee"
  ...
  val mundaneVersion = "1.2.1-20150309045107-feef53f"
  ...
  def poacher(version: String) =
    if (version.contains("cdh4"))
      Seq("com.ambiata" %% "poacher" % "1.0.0-cdh4-20150309055122-87a87d8" % "compile->compile;test->test") ++ hadoop(version)
    else if (version.contains("cdh5"))
      Seq("com.ambiata" %% "poacher" % "1.0.0-cdh5-20150309055121-87a87d8" % "compile->compile;test->test") ++ hadoop(version)
    else
