...
object depend {
  ...
  val sawsVersion = "1.2.1-20150309015907-d64c646"
  ...
  val mundaneVersion = "1.2.1-20150309015053-827ae37"
  ...
  def poacher(version: String) =
    if (version.contains("cdh4"))
      Seq("com.ambiata" %% "poacher" % "1.0.0-cdh4-20150309015835-d93de08" % "compile->compile;test->test") ++ hadoop(version)
    else if (version.contains("cdh5"))
      Seq("com.ambiata" %% "poacher" % "1.0.0-cdh5-20150309015833-d93de08" % "compile->compile;test->test") ++ hadoop(version)
    else
