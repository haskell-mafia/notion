...
import com.ambiata.notion.store
...
class HdfsStoreSpec extends Specification with ScalaCheck { def is = sequential ^ s2"""
  ...
  implicit def HdfsStoreArbitary: Arbitrary[store.HdfsStore] =
  ...
  def list =
    prop((store: store.HdfsStore, keys: Keys) => clean(store, keys) { keys =>
       store.list(Key.Root) must beOkLike((_:List[Key]) must contain(exactly(keys:_*))) })

  def listFromPrefix =
    prop((store: store.HdfsStore, keys: Keys) => clean(store, keys.map(_ prepend "sub")) { keys =>
  ...
  def find =
    prop((store: store.HdfsStore, keys: Keys) => keys.keys.length >= 3 ==> { clean(store, keys) { keys =>
  ...
  def notExists =
    prop((store: store.HdfsStore, keys: Keys) => store.exists(Key.Root / "missing") must beOkValue(false))
  ...
  def move =
    prop((store: store.HdfsStore, m: KeyEntry, n: KeyEntry) => clean(store, Keys(m :: Nil)) { _ =>
      (store.move(m, n) >>
  ...
  def moveTo =
    prop((store: store.HdfsStore, alternate: store.HdfsStore, m: KeyEntry, n: KeyEntry) => clean(store, alternate, Keys(m :: Nil)) { _ =>
  ...
  def mirrorTo =
    prop((store: store.HdfsStore, alternate: store.HdfsStore, keys: Keys) => clean(store, alternate, keys) { keys =>
      store.mirrorTo(alternate, Key.Root, Key("mirror")) >> alternate.list(Key("mirror")) must
        beOkLike((_:List[Key]) must contain(exactly(keys:_*))) })

  def checksum =
    prop((store: store.HdfsStore, m: KeyEntry) => clean(store, Keys(m :: Nil)) { _ =>
      store.checksum(m, MD5) must beOkValue(Checksum.string(m.value.toString, MD5)) })

  def bytes =
    prop((store: store.HdfsStore, m: KeyEntry, bytes: Array[Byte]) => clean(store, Keys(m :: Nil)) { _ =>
      (store.bytes.write(m, ByteVector(bytes)) >> store.bytes.read(m)) must beOkValue(ByteVector(bytes)) })

  def strings =
    prop((store: store.HdfsStore, m: KeyEntry, s: String) => clean(store, Keys(m :: Nil)) { _ =>
  ...
  def lines =
    prop((store: store.HdfsStore, m: KeyEntry, s: List[Int]) => clean(store, Keys(m :: Nil)) { _ =>
  ...
  def create(store: store.HdfsStore, keys: Keys): ResultT[IO, Unit] =
  ...
  def clean[A](store: store.HdfsStore, keys: Keys)(run: List[Key] => A): A = {
    create(store, keys).run.unsafePerformIO
    try run(files(keys))
    finally store.deleteAll(Key.Root).run.void.unsafePerformIO
  }

  def clean[A](store: store.HdfsStore, alternate: store.HdfsStore, keys: Keys)(run: List[Key] => A): A = {
    create(store, keys).run.unsafePerformIO
    try run(files(keys))
